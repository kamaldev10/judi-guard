...
const getAuthenticatedYouTubeClient = async (userId) => {
  const user = await UserRepository.findById(userId).select(
  ...
  if (needsRefresh) {
    ...
    try {
      console.log(`[YouTubeService] Refreshing YouTube access token for user ${userId}...`);
...
const getClient = (tokens) => {
  const client = googleOAuth2Client(config.youtube.guestRedirectUri);
...
const getChannelVideos = async (tokens, pageToken = '') => {
  try {
    ...
    if (error.code === 401 || error.message.includes('invalid_grant')) {
      throw new AppError('Sesi YouTube kadaluarsa. Silakan hubungkan ulang.', 401);
    }
 ...
 */
const getYoutubeVideoComments = async (tokens, videoId, pageToken = '') => {
  try {
    const youtube = getClient(tokens);
    ...
    if (error.code === 403) {
      throw new AppError('Komentar dinonaktifkan pada video ini atau kuota habis.', 403);
    }
...
const getAllComments = async (tokens, videoId) => {
  try {
    ...
    console.error('[YouTube Service] Error fetching all comments:', error.message);
    ...
    throw new AppError(`Gagal mengambil seluruh komentar: ${error.message}`, 502);
  }
};
 ...
 */
const setModerationStatus = async (tokens, commentIds, status = 'rejected', banAuthor = false) => {
  try {
    const youtube = getClient(tokens);
    ...
    console.error('[YouTube Service] Critical Error setModerationStatus:', error.message);
    throw new AppError(`Gagal melakukan moderasi ke YouTube: ${error.message}`, 502);
  }
};
...
const getChannelInfoByHandle = async (tokens, handle) => {
  ...
  if (!handle) {
    throw new Error('Parameter handle kosong atau undefined saat memanggil service.');
  }
  ...
  try {
    ...
    if (!items || items.length === 0) {
      throw new AppError(`Channel dengan handle '${handle}' tidak ditemukan.`, 404);
    }
...
const getVideoDetails = async (videoId, { youtubeClient, apiKey }) => {
  ...
  if (!authClient) {
    if (apiKey) {
      ...
      console.log(`[YouTubeService] Mengambil detail video ${videoId} menggunakan API Key.`);
    } else {
      throw new AppError(
  ...
  try {
    ...
    throw new NotFoundError(`Video dengan ID ${videoId} tidak ditemukan atau tidak dapat diakses.`);
  } catch (error) {
    console.error(
    ...
    if (error.code === 403 && error.message && error.message.includes('quotaExceeded')) {
      throw new AppError('Kuota YouTube API telah terlampaui. Silahkan coba lagi esok hari', 429); // Too Many Requests
    }

    if (error.code === 404) {
      throw new NotFoundError(`Video dengan ID ${videoId} tidak ditemukan (API Error).`);
    }
    throw new AppError(
...
const fetchCommentsForVideo = async (
  ...
  try {
    do {
      const resultsToFetchThisPage = Math.min(actualMaxPerPage, limitTotalResults - fetchedCount);
    ...
    if (error.code === 403) {
      ...
      if (googleApiErrorMessage && googleApiErrorMessage.toLowerCase().includes('quotaexceeded')) {
        throw new AppError('Kuota YouTube API telah terlampaui.', 429);
      }
      throw new AppError(`Akses ditolak untuk mengambil komentar: ${googleApiErrorMessage}`, 403);
    }
    throw new AppError(
...
const deleteYoutubeComment = async (youtubeCommentId, { youtubeClient }) => {
  try {
    ...
    if (!authenticatedUserChannelId || commentAuthorChannelId !== authenticatedUserChannelId) {
 ...
 */
const moderateYoutubeComment = async (youtubeCommentId, moderationStatus, { youtubeClient }) => {
  try {
