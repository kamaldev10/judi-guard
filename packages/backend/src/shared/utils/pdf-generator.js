...
const generateModerationReport = (analysisData, comments, res) => {
  ...
  doc.text(`Sukses Dihapus            : ${analysisData.lastBatchDeletionSuccessCount || 0}`);
  doc.moveDown();

  // 5. Daftar Spam (Limit 50 item agar tidak terlalu panjang, atau unlimited)
  doc.fontSize(12).font('Helvetica-Bold').text('DAFTAR KOMENTAR SPAM (Preview)');
  doc.moveDown();
  ...
  // 6. Footer
  doc.moveDown(2);
  doc.fontSize(8).text('Judi Guard Automated Report System', { align: 'center' });
...
const generatePeriodReport = (reportData, res) => {
  ...
  reportData.details.forEach((video) => {
    ...
    if (!isClean && video.spamComments && video.spamComments.length > 0) {
      ...
      video.spamComments.forEach((comment) => {
        const authorName = (comment.commentAuthorDisplayName || 'Unknown').substring(0, 25);
        ...
        if (doc.y + rowHeight > 750) {
          doc.addPage();
          doc.fontSize(8).font('Helvetica-Bold').text('(Lanjutan Tabel...)', 50, 50);
          doc.y = 70;
        }
  ...
  doc.fontSize(8).fillColor('grey').text('Dicetak oleh Judi Guard Automated System', 50, bottom, {
    align: 'center',
  });
