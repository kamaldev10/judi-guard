...
Given('I am logged in as {string}', (email) => {
  const password = email === authData?.validUser?.email ? authData.validUser.password : 'Admin123';
...
Then('I should see a loading notification with the text {string}', (message) => {
...
Then('I should see a success notification with the text {string}', (message) => {
...
Then('I should see a validation error message containing {string}', (message) => {
  cy.contains(message).should('be.visible');
});
