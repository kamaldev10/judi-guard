...
export default function AnalysisSelection() {
  ...
  useEffect(() => {
    // Cek myVideos.length agar tidak fetch ulang jika kembali dari Preview
    if (myVideos.length === 0) {
      fetchMyVideos();
    }
  }, [fetchMyVideos, myVideos.length]);

  const onSearch = (e) => {
    e.preventDefault();
    handleSearchVideo(inputUrl);
  };
    ...
    <div className="mx-auto max-w-6xl space-y-10 py-8 animate-in fade-in duration-500">
      ...
      <div>
        <div className="mb-6 flex items-center justify-between">
          ...
          {/* Bisa tambah filter/sort disini nanti */}
        </div>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {myVideos.map((video) => (
              <div
                ...
                key={video.id}
                onClick={() => selectVideoForPreview(video)}
              ...
              >
                ...
                <div className="relative aspect-video w-full overflow-hidden bg-gray-200">
                  ...
                  {/* Duration Badge (Kalau ada datanya) */}
                  {/* <span className="absolute bottom-2 right-2 rounded bg-black/80 px-1.5 py-0.5 text-xs font-bold text-white">10:24</span> */}
                </div>
            ...
            {/* Loading Skeletons (Muncul saat isLoadingList true) */}
            {isLoadingList &&
              [1, 2, 3, 4].map((i) => (
                <div
                  key={`skel-${i}`}
