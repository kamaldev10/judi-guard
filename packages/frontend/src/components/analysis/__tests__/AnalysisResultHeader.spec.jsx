...
describe('AnalysisResultHeader Component', () => {
  ...
  it('should render correctly for COMPLETED status', () => {
    render(<AnalysisResultHeader analysisId={mockAnalysisId} videoData={mockVideoDataCompleted} />);
    ...
    expect(screen.getByText(mockVideoDataCompleted.videoTitle)).toBeInTheDocument();
    ...
    expect(screen.queryByText(/status analisis saat ini/i)).not.toBeInTheDocument();
  });
  ...
  it('should display youtubeVideoId if videoTitle is missing', () => {
    ...
    expect(screen.getByText(mockVideoDataNoTitle.youtubeVideoId)).toBeInTheDocument();
  ...
  it('should render correctly for PROCESSING status with default polling message', () => {
    render(
      <AnalysisResultHeader analysisId={mockAnalysisId} videoData={mockVideoDataProcessing} />,
    );
    ...
    expect(statusBadge).toBeInTheDocument();
    expect(statusBadge).toHaveClass('bg-orange-100', 'text-orange-700', 'animate-pulse');
  ...
  it('should render correctly for PROCESSING status with custom polling message', () => {
    ...
    expect(screen.getByText(new RegExp(customPollingMsg, 'i'))).toBeInTheDocument();
  ...
  it('should render correctly for FAILED status with errorMessage', () => {
    ...
    expect(screen.getByText(mockVideoDataFailed.errorMessage)).toBeInTheDocument(); // Cek pesan error spesifik
    expect(screen.queryByText(/status analisis saat ini/i)).not.toBeInTheDocument(); // Pesan processing tidak ada
  });
  ...
  it('should render correctly for ERROR status without errorMessage', () => {
    render(
      <AnalysisResultHeader analysisId={mockAnalysisId} videoData={mockVideoDataErrorStatus} />,
    );
