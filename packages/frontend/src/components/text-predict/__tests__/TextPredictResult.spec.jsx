...
describe("Text Predict Result Component Testing", () => {
  ...
  it("should render the loading message when isLoading is true", () => {
    ...
    // Pastikan state lain tidak ter-render
    expect(
      screen.queryByText(/tidak ada teks yang diprediksi/i)
  ...
  describe("when a prediction is provided", () => {
    ...
    it("should render all prediction data correctly", () => {
      ...
      expect(
        screen.getByText(`Model: ${judiPrediction.modelVersion}`)
      ).toBeInTheDocument();

      // Verifikasi klasifikasi
      expect(
        screen.getByText(judiPrediction.classification)
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByRole("button", { name: /reset hasil/i })
      ).toBeInTheDocument();
    });
