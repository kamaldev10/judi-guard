...
describe('Auth API Service Unit Testing', () => {
  ...
  describe('registerUserApi', () => {
    ...
    it('should call apiClient.post with correct payload and return data', async () => {
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/auth/register', expectedPayload);
    ...
    it('should throw the default error from handleApiError on failure', async () => {
      ...
      await expect(registerUserApi(userData)).rejects.toThrow('Gagal melakukan pendaftaran.');
    });
  });
  ...
  describe('signInWithGoogleApi', () => {
    ...
    it('should throw the default error from handleApiError on failure', async () => {
      apiClient.post.mockRejectedValue(new Error('Network Error'));
      await expect(signInWithGoogleApi(idToken)).rejects.toThrow('Login Google gagal.');
    });
  });
  ...
  describe('verifyOtpApi', () => {
    ...
    it('should throw the specific error from handleApiError on failure', async () => {
      ...
      await expect(verifyOtpApi(email, otpCode)).rejects.toThrow(specificMessage);
    });
  });
  ...
  describe('resendOtpApi', () => {
    ...
    it('should throw the default error from handleApiError on failure', async () => {
      apiClient.post.mockRejectedValue(new Error('Network Error'));
      await expect(resendOtpApi(email)).rejects.toThrow('Gagal mengirim ulang OTP.');
    });
  });
});
