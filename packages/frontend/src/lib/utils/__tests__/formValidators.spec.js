...
describe('validateUserName', () => {
  ...
  it('harus mengembalikan error jika username terlalu panjang', () => {
    ...
    expect(validateUserName(longUsername)).toBe('Username maksimal 30 karakter.');
  });
...
describe('validateEmail', () => {
  ...
  it('harus mengembalikan error untuk format email tidak valid', () => {
    ...
    expect(validateEmail('tes contoh@domain.com')).toBe('Format email tidak valid.');
...
describe('validateRegistrationPassword', () => {
  ...
  it('harus mengembalikan error jika password kosong', () => {
    expect(validateRegistrationPassword('')).toBe('Password tidak boleh kosong.');
    // Catatan: Fungsi ini tidak melakukan trim, jadi spasi dihitung
  });

  it('harus mengembalikan error jika password terlalu pendek', () => {
    expect(validateRegistrationPassword('aBc1')).toBe(`Password minimal ${minLength} karakter.`);
  });
...
describe('validateYoutubeUrl', () => {
  it('harus mengembalikan null untuk URL YouTube valid', () => {
    ...
    expect(validateYoutubeUrl('http://www.youtube.com/watch?v=dQw4w9WgXcQ')).toBeNull();
    expect(validateYoutubeUrl('www.youtube.com/watch?v=dQw4w9WgXcQ')).toBeNull();
  ...
  it('harus mengembalikan error jika URL kosong, null, undefined, atau bukan string', () => {
    expect(validateYoutubeUrl('')).toBe('URL video YouTube tidak boleh kosong.');
    expect(validateYoutubeUrl('   ')).toBe('URL video YouTube tidak boleh kosong.');
    expect(validateYoutubeUrl(null)).toBe('URL video YouTube tidak boleh kosong.');
    expect(validateYoutubeUrl(undefined)).toBe('URL video YouTube tidak boleh kosong.');
    expect(validateYoutubeUrl(123)).toBe('URL video YouTube tidak boleh kosong.');
  });
  ...
  it('harus mengembalikan error untuk format URL tidak valid', () => {
    ...
    expect(validateYoutubeUrl('http://youtubee.com/watch?v=abc')).toBe(invalidFormatMsg); // Typo domain
  });
});
