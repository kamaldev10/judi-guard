...
const OtpForm = ({ email }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await verifyOtpMutation.mutateAsync({ email, otpCode: enteredOtp });

      // Google user — perlu set password
      if (res?.status === 'set_password_required') {
        toast.success('Verifikasi OTP berhasil! Silakan buat password.', {
          position: 'bottom-right',
          toastId: 'toast-otp-verified',
        });
        const tempToken = res?.data?.token;
        setTimeout(() => {
          // ponytail: tempToken via location.state, backend tidak require di header
          navigate('/set-password', { state: { email, tempToken } });
        }, 0);
        return;
      }

      // Regular user — sudah auto-login via onSuccess
      ...
      <div className="flex justify-center gap-2 mb-6">
        {otp.map((digit, i) => (
          <input
            ...
            inputMode="numeric"
            maxLength="1"
            className="w-12 h-12 text-center text-2xl bg-white border-2 border-[#6148FF] rounded-2xl focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-black"
            value={digit}
