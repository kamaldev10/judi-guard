...
import { downloadReportPdf } from '../services/analysis.api.js';
...
export default function AnalysisResults() {
  ...
  const filters = useAnalysisUiStore((state) => state.filters);

  const setFilter = useAnalysisUiStore((state) => state.setFilter);
  const setPage = useAnalysisUiStore((state) => state.setPage);
  const resetToSelection = useAnalysisUiStore((state) => state.resetToSelection);

  const { data: resultsData, isLoading: isLoadingResults } = useAnalysisResultsQuery(
    activeAnalysisId,
    filters,
  );

  const executeActionMutation = useExecuteActionMutation();
  const executeUndoMutation = useUndoActionMutation();

  const comments = resultsData?.comments || [];
  const pagination = resultsData?.pagination || null;
  ...
  const isExecuting = executeActionMutation.isPending || executeUndoMutation.isPending;
  ...
  const handleDeleteConfirm = async () => {
    if (selectedIds.length === 0 || !activeAnalysisId) return;
    ...
    try {
      await executeActionMutation.mutateAsync({
        ...
        payload: {
          commentIds: idsToDelete,
          action: 'DELETE',
          banAuthor,
        },
      });

      queryClient.invalidateQueries({ queryKey: ['analysis-results', activeAnalysisId] });
      ...
      toast.success(`Berhasil menghapus ${count} komentar`, {
        ...
        action: {
          label: 'UNDO SEKARANG',
          onClick: () => {
            const promise = executeUndoMutation
              .mutateAsync({
                analysisId: activeAnalysisId,
                commentIds: idsToDelete,
              })
  ...
  const handleUndo = async () => {
    if (selectedIds.length === 0 || !activeAnalysisId) return;
    try {
      await executeUndoMutation.mutateAsync({
        analysisId: activeAnalysisId,
      ...
      queryClient.invalidateQueries({ queryKey: ['analysis-status', activeAnalysisId] });
      setSelectedIds([]);
      toast.success('Komentar berhasil dikembalikan!');
    } catch (err) {
      toast.error(err.message || 'Gagal mengembalikan komentar');
    }
  };
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const blob = await downloadReportPdf(activeAnalysisId);
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `report-${activeAnalysisId}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast.success('Laporan PDF berhasil diunduh.');
    } catch (err) {
      toast.error(err.message || 'Gagal mengunduh laporan PDF.');
    }
  };
