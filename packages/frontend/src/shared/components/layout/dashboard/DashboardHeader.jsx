...
import {
  ...
  useDisconnectYoutubeGuestMutation,
...
export default function DashboardHeader() {
  ...
  const { data: channelProfile } = useYoutubeChannelQuery();
  const connectMutation = useConnectGoogleMutation();
  const disconnectMutation = useDisconnectYoutubeGuestMutation();
      ...
      <div className="flex items-center gap-4">
        ...
        {isConnected && channelProfile ? (
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-full bg-red-600 px-4 py-2 text-xs font-bold text-white shadow-md transition-all hover:bg-red-700 hover:shadow-lg disabled:opacity-50"
          >
            <Youtube size={14} />
            {connectMutation.isPending ? 'Menghubungkan...' : 'Hubungkan Channel'}
          </button>
        )}
      </div>
    </header>
