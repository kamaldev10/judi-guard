...
describe('Not Login Component Testing', () => {
  ...
  it('should render the descriptive paragraphs', () => {
    ...
    expect(screen.getByText(/untuk mengakses fitur ini, anda perlu login/i)).toBeInTheDocument();
    expect(screen.getByText(/silakan kembali ke halaman login atau daftar/i)).toBeInTheDocument();
  });
  ...
  it('should not render the login button (since it is commented out)', () => {
    ...
    expect(screen.queryByRole('button', { name: /login sekarang/i })).not.toBeInTheDocument();
  });
});
