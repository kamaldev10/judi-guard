...
describe('useVideoAnalysisStore unit testing', () => {
  ...
  it('should handle submitVideoForAnalysis success', async () => {
    ...
    vi.mocked(videoAnalysisApi.submitVideoForAnalysisApi).mockResolvedValue(mockData);

    // Panggil action
    await useVideoAnalysisStore.getState().submitVideoForAnalysis('http://youtube.com/video');
  ...
  it('should call batchDelete AND then refresh comments on success', async () => {
    ...
    // Mock Delete API
    vi.mocked(videoAnalysisApi.batchDeleteJudiCommentsApi).mockResolvedValue(deleteResult);
    // Mock Fetch API (karena dipanggil setelah delete sukses)
    vi.mocked(videoAnalysisApi.getAnalyzedCommentsApi).mockResolvedValue(refreshedComments);
    ...
    expect(videoAnalysisApi.batchDeleteJudiCommentsApi).toHaveBeenCalledWith(analysisId);

    // 2. Pastikan Fetch API dipanggil SETELAH delete (untuk refresh UI)
    expect(videoAnalysisApi.getAnalyzedCommentsApi).toHaveBeenCalledWith(analysisId);
  ...
  it('should call deleteSingle AND then refresh comments', async () => {
    ...
    await useVideoAnalysisStore.getState().deleteSingleComment(commentId, analysisId);

    expect(videoAnalysisApi.deleteSingleCommentApi).toHaveBeenCalledWith(commentId);
    expect(videoAnalysisApi.getAnalyzedCommentsApi).toHaveBeenCalledWith(analysisId);
    expect(useVideoAnalysisStore.getState().analyzedComments).toEqual(refreshedComments);
  });
